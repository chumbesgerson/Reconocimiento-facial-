%Proyecto_Emociones.m - VERSIÓN CORREGIDA
%Script único que realiza todo el pipeline FER2013 CNN plus MLP y la interfaz
...
if ~modeloCargado
    ...
    options = trainingOptions('adam', ...
        ...
        'MaxEpochs', 20, ...
        'MiniBatchSize', 64, ... % Reducido para evitar problemas de memoria
        'Shuffle', 'every-epoch', ...
...
try
    % Intentar inicializar la cámara con diferentes configuraciones
    cam = [];
    maxAttempts = 3;
    ...
    for attempt = 1:maxAttempts
        try
            ...
            % Opción 1: Usar webcam sin parámetros
            if attempt == 1
                cam = webcam(1); % Índice 1
                cam.Resolution = '640x480';
            
            % Opción 2: Usar nombre específico
            elseif attempt == 2
                cam = webcam(camList{1}); % Por nombre
                cam.Resolution = '320x240';
            
            % Opción 3: Probar sin especificar resolución
            elseif attempt == 3
                cam = webcam(1);
                % Dejar resolución por defecto
            end
            ...
            pause(1); % Esperar 1 segundo entre intentos
        end
    end
    ...
    function capturePhoto(~, ~)
        ...
        imwrite(frameOut, fullfile(projectRoot, 'resultados', photoName));
